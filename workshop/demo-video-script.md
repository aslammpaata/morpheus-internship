...
- [ ] `morctl.exe` already **built and tested**; a terminal open in its folder.
...
| **1:50–3:00 — Tier 2: developers** | Terminal: run `morctl --dev --prompt "Hello!"`. Show the output lines: ✔ router reachable → wallet balance → ✔ session opened → ✔ inference ok → reply. | "For developers: this one command opens a paid session, grabs the session ID, and runs inference — the four manual API calls, collapsed into one. And the API is OpenAI-compatible, so your existing code just points at Morpheus." |
...
- Export: MP4 (H.264), plus a muted GIF of the `morctl --dev` run for the repo README.
