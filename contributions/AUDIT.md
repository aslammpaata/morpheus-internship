...
| **F16** | **Desktop App onboarding completely broken** | Fresh install, no prior state, wallet setup fails unconditionally: "Failed to finish onboarding." Confirmed on multiple machines and multiple versions (v7.3.0 and an earlier tested release). Also reproduces when switching an existing wallet on an already-onboarded install. Blocks every new user at step one. | **Critical** | Open — [#811](https://github.com/MorpheusAIs/Morpheus-Lumerin-Node/issues/811), booth-blocking if unresolved |
